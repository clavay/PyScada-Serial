...
class SerialDevice(models.Model):
    serial_device = models.OneToOneField(
    ...
    timeout = models.PositiveSmallIntegerField(
        default=0, help_text="0 use default, else value in seconds"
    )
    stopbits_choices = (
        (1, "one stopbit"),
    ...
    bytesize = models.PositiveSmallIntegerField(
        default=serial.EIGHTBITS, choices=bytesize_choices
    )
    parity_choices = (
    ...
    parity = models.CharField(
        default=serial.PARITY_NONE, max_length=254, choices=parity_choices
    )
...
class SerialVariable(models.Model):
    serial_variable = models.OneToOneField(
